...
void ChangeSceneButton::onMouseMove(sf::Event event)
{
...
void ChangeSceneButton::onMouseUp(sf::Event event) 
{
    if (event.mouseButton.button == sf::Mouse::Left)
    {
        if (m_isMouseOnButton == true)
        {
            Engine::Window::get().switchScene(m_changeToScene);
        }
    }
}

void ChangeSceneButton::draw(sf::RenderTarget &target, sf::RenderStates states) const
{
...
ChangeSceneButton::ChangeSceneButton(std::string text, int size) 
{
    m_buttonName = text;
    m_buttonSize = size;
}

void ChangeSceneButton::setChangeToScene(Engine::Scene &scene)
{
